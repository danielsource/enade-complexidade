...
para descobrir quanto tempo esse algoritmo de ordenação gasta, uma forma seria rodá-lo em um computador.
...
com tarefas diversas rodando em plano de fundo em um sistema operacional;
...
Inevitavelmente, mesmo com grandes constantes, um algoritmo no pior caso com $\Theta(n^2)$ vai ser mais eficiente que outro com $\Theta(n^3)$.

Assim, é possível estudar a complexidade assintótica de algoritmos.
Isto é, analisar como o custo computacional aumenta à medida que o tamanho da entrada tende para o infinito.
...
ou seja, essa notação define o limite inferior \parencite[49--51, 57]{clrs2022}.
...
Além disso, é inútil dizer que ``\textit{insertion sort} \emph{é} $O(n^2)$''; por mais que isso seja verdade,
poderíamos ter usado qualquer outro $O(n^c)$ em que $c \ge 2$ \parencite[57]{clrs2022}.
