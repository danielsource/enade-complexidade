...
Quando um algoritmo de divisão e conquista divide um problema de tamanho $n$ em $a$ subproblemas de tamanho $n/b$,
...
quando o tamanho do subproblema diminui apenas por uma \emph{quantidade} constante (como em $T(n-1)$),
em vez de por um \emph{fator} constante (como em $T(n/2)$),
...
no número de folhas da árvore \parencite[seç.~4.1]{clrs2022}.
...
\]
Vale notar, como discutido na análise do \textit{Algoritmo 2} da Questão 2014-12 (seção~\ref{sec:assintotica}),
que \texttt{fib} não possui múltiplos casos a serem analisados separadamente:
diferentemente do \textit{Linear-Search} (seção~\ref{sec:casos}), cujo tempo depende de \emph{onde} o valor buscado
está no vetor, o tempo de \texttt{fib} depende exclusivamente do \emph{tamanho} $n$ da entrada,
...
em que o tamanho do subproblema é reduzido por um \emph{fator} constante (como em $n/2$),
aqui o tamanho é reduzido apenas por uma \emph{quantidade} constante ($n-1$ e $n-2$).
Esse é o mesmo padrão da recorrência da busca linear recursiva citada por \textcite[seç.~4.1]{clrs2022},
$T(n) = T(n-1) + \Theta(1)$, cuja árvore de recursão tem altura $\Theta(n)$.
No caso de \texttt{fib}, porém, cada chamada gera \emph{duas} novas chamadas (fator de ramificação 2),
...
Assim como no exemplo do polinômio de 50 linhas discutido na seção~\ref{sec:assintotica},
o termo aditivo $\Theta(1)$ da recorrência é desprezível diante do crescimento exponencial das chamadas recursivas,
não alterando a ordem de crescimento final.
...
resultando em tempo $O(n)$ e espaço extra $O(1)$.
...
Assim como no \textit{Sum-Array} (seção~\ref{sec:modelo}), em que um laço que itera $n$ vezes com custo constante
por iteração contribui com um termo $\Theta(n)$ ao tempo total,
o laço da linha 3 de F itera $n$ vezes chamando \texttt{G(i)} a custo constante por chamada, contribuindo com $\Theta(n)$.
Somando esse custo ao da chamada recursiva da linha 6, a recorrência que descreve o tempo de execução de F é
\[
    T(n) = T(n/2) + \Theta(n).
\]

A afirmação I é falsa.
Essa é uma recorrência mestre \parencite[seç.~4.5]{clrs2022} da forma $T(n) = aT(n/b) + f(n)$
com $a = 1$ (apenas uma chamada recursiva) e $b = 2$.
Já a recorrência do \textit{mergesort}, apresentada por \textcite[seç.~2.3.2]{clrs2022},
é $T(n) = 2T(n/2) + \Theta(n)$, com $a = 2$ (duas chamadas recursivas).
São recorrências estruturalmente diferentes~---~F ramifica em apenas um subproblema por chamada,
enquanto o \textit{mergesort} ramifica em dois~---~e, consequentemente, têm soluções assintóticas diferentes:
para F, a função \emph{watershed} do método mestre é $n^{\log_2 1} = n^0 = 1$;
como $f(n) = \Theta(n)$ cresce polinomialmente mais rápido que essa \emph{watershed},
aplica-se o caso~3 do teorema mestre, resultando em $T(n) = \Theta(n)$.
Já para o \textit{mergesort}, a \emph{watershed} é $n^{\log_2 2} = n$,
que coincide assintoticamente com $f(n) = \Theta(n)$, aplicando-se o caso~2, com solução $\Theta(n \log n)$.
Note que, como discutido na seção~\ref{sec:assintotica} a respeito da ordem de crescimento,
não basta observar que ambas as recorrências contêm um termo $\Theta(n)$: é a forma da recursão
(o valor de $a$) que determina se a ordem de crescimento final é linear ou log-linear.

A afirmação II é verdadeira.
Assim como discutido por \textcite[seç.~4.4--4.5]{clrs2022} a respeito da altura de árvores de recursão,
quando o tamanho do problema é dividido por um fator constante $b$ a cada chamada~---~no caso de F,
o argumento é dividido por 2 (\texttt{F(n/2)})~---~o número de níveis de recursão necessários até o argumento
atingir a condição de parada ($n > 0$ falso) é $\Theta(\log_b n) = \Theta(\log n)$.

A afirmação III é verdadeira.
Utilizando a técnica de árvore de recursão descrita por \textcite[seç.~4.4]{clrs2022},
no nível $k$ da recursão (correspondente à chamada \texttt{F(n/2\^{}k)}) o laço da linha 3 realiza
$\Theta(n/2^k)$ chamadas a G.
Como há $\Theta(\log n)$ níveis (pela afirmação II), o número total de chamadas a G é majorado por
\[
    \sum_{k=0}^{\Theta(\log n)} \frac{n}{2^k} \;\le\; \Theta(\log n) \cdot n = O(n \log n).
\]
Esse é o mesmo tipo de raciocínio~---~somar o custo por nível e multiplicar pela altura da árvore~---~empregado
por \textcite[seç.~4.4]{clrs2022} na análise da recorrência irregular $T(n) = T(n/3) + T(2n/3) + \Theta(n)$.
Vale notar que essa cota, embora correta, não é a mais justa: como os termos formam uma série geométrica
decrescente ($n + n/2 + n/4 + \dots$), o total real converge para $\Theta(n)$, que está contido em $O(n \log n)$~---~
o mesmo cuidado sobre limites superiores frouxos discutido na seção~\ref{sec:assintotica} a respeito do \textit{insertion sort},
que também admite ser descrito por $O(n^2)$, $O(n^3)$ etc., sem que essas cotas sejam as mais justas.

Como as afirmações II e III são verdadeiras, e a I é falsa, temos que D é a opção correta.
