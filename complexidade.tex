...
\begin{tikzpicture}[
    ...
    every node/.style={circle, draw, thick, fill=yellow!80, minimum size=8mm, inner sep=1pt, font=\sffamily\large},
...
\node {15}
    child {node {10}
        child {node {7}}
        child {node {13}
            child[missing] % Espaço vazio para manter o 14 à direita
            child {node {14}}
        }
    }
    child {node {20}
...
Para um melhor entendimento, é indicada a leitura do artigo \textit{\citefield{article}{title}} \parencite{article}.

A afirmação I é verdadeira. A árvore binária de pesquisa permite a busca eficiente semelhante à busca binária em um vetor ordenado, pois a cada comparação reduzimos pela metade o espaço de busca.

A afirmação II é falsa. A árvore está balanceada, pois a subárvore da esquerda possui 4 nós e a subárvore da direita possui 3 nós, e uma árvore binária apenas fica desbalanceada quando a diferença entre o número de nós das subárvores esquerda e direita é maior que 1.

A afirmação III é falsa. O caminhamento pós-ordem visita os nós na ordem: filhos da esquerda, filhos da direita, e depois o valor do seu nó. Portanto, os valores serão impressos na seguinte ordem: 7, 14, 13, 10, 17, 25, 20, 15.

A afirmação IV é verdadeira. O número de comparações em uma busca binária realizada com sucesso é $O(\log n)$, pois a cada comparação o espaço de busca é reduzido pela metade, o que significa que, se a árvore duplicasse de tamanho, seria necessária apenas mais uma comparação para reduzir seu espaço de busca de volta.
...
\begin{enumerate}
    \item Crie $n+1$ conjuntos unitários, cada um contendo um dos pontos a serem ligados entre si e insira esses conjuntos em um conjunto $C$.
    \item Crie um conjunto $D$ contendo um registro para cada combinação possível de dois pontos distintos a serem ligados. Cada registro deve conter os campos $c_i$, $c_j$ e $d$, em que $c_i$ e $c_j$ são os dois pontos a serem ligados e $d$ é a distância entre eles.
...
\begin{enumerate}
    \item[(A)] o problema exemplifica a obtenção de uma árvore geradora mínima, portanto está no conjunto $P$.
    \item[(B)] o algoritmo é uma heurística para o Problema do Caixeiro Viajante, logo apresenta complexidade polinomial.
    \item[(C)] o problema descrito é de otimização, logo pertence ao conjunto $NP$-difícil, mas não ao conjunto $NP$-completo.
    \item[(D)] uma alternativa para a solução do problema é usar o algoritmo de Dijkstra para obtenção do caminho mínimo entre dois pontos.
    \item[(E)] o passo de maior custo do algoritmo é a criação do conjunto $D$ com as combinações de pontos, apresentando complexidade computacional $O(n!)$.
\end{enumerate}
\end{question}

O problema descrito consiste em um conjunto com $n+1$ pontos, sendo $n$ o número de bebedouros e $1$ o poço artesiano. O objetivo é encontrar a forma de conectar todos os pontos com o menor custo, minimizando a extensão total dos encanamentos. Esse problema é modelado como o problema da Árvore Geradora Mínima (MST) e é resolvido eficientemente por algoritmos como o de Kruskal e o de Prim, ambos com complexidade polinomial, pertencentes à classe de complexidade $P$. Ambos os algoritmos são do tipo guloso (escolhem a melhor opção local a cada passo e, para o problema da MST, garantem a solução ótima global). O algoritmo de Prim possui complexidade $O(E + V \log V)$ e o de Kruskal possui complexidade $O(E \log V)$, em que $E$ é o número de arestas e $V$ é o número de vértices do grafo. Para uma explicação mais detalhada, consultar o capítulo 23, página 525 do livro \textit{\citefield{clrs2022}{title}} \parencite{clrs2022}.

A opção A está correta, pois o problema exemplifica a obtenção de uma árvore geradora mínima, portanto está no conjunto $P$.

A opção B está incorreta, pois o algoritmo descrito não é uma heurística para o Problema do Caixeiro Viajante; ademais, o Caixeiro Viajante é um problema NP-difícil na versão de otimização, logo não é possível encontrar uma solução ótima em tempo polinomial no pior caso.

A opção C está incorreta. O problema descrito é de otimização, mas pertence ao conjunto $P$, e não ao conjunto NP-difícil, uma vez que é possível encontrar a solução ótima em tempo polinomial.

A opção D está incorreta, pois o algoritmo de Dijkstra é utilizado para encontrar o caminho mínimo entre dois pontos, logo não resolve o problema da árvore geradora mínima, uma vez que é preciso encontrar uma solução que conecte todos os pontos com o menor custo total.

A opção E está incorreta. A criação do conjunto $D$ é realmente a parte de maior custo do algoritmo, mas o processo de criar as combinações de pontos tem complexidade $O(n^2)$, e não $O(n!)$, visto que a ordem dos pares não importa — ou seja, $(A, B)$ ou $(B, A)$ —, logo o número de combinações é dado por $C(n+1, 2) = \frac{(n+1)!}{2!(n-1)!} = \frac{(n+1)n}{2} = O(n^2)$.
