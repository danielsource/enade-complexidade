...
\begin{document}
\maketitle
1ª atividade da disciplina optativa ENADE-2026.2, do Professor Ricardo Azevedo Moreira da Silva.
...
Porém, o algoritmo será mais rápido em um computador mais potente~---
que consegue executar mais instruções ou processar mais dados~---
em comparação a um computador menos potente.
...
e baseado no caso anterior, o laço interno (linhas 5--7) será executado apenas quando $i = n$;
...
\begin{question}{2014-13}
A figura a seguir apresenta uma árvore binária de pesquisa, que mantém a seguinte propriedade fundamental:
...
\begin{tikzpicture}[
    ...
    level/.style={sibling distance=3.5cm/(1.5^(#1-1))},
    every node/.style={circle, draw, thick, fill=yellow!80, minimum size=6mm, inner sep=1pt, font=\sffamily},
...
\begin{enumerate}[label=\Roman*.]
    \item A árvore possui a vantagem de realizar a busca de elementos de forma eficiente, como a busca binária em um vetor.
    \item A árvore está desbalanceada, pois a subárvore da esquerda possui um número de nós maior do que a subárvore da direita.
    \item Quando a árvore é percorrida utilizando o método de caminhamento pós-ordem, os valores são encontrados em ordem decrescente.
    \item O número de comparações realizadas em função do número $n$ de elementos na árvore em uma busca binária realizada com sucesso é $O(\log n)$.
\end{enumerate}
...
A árvore binária de pesquisa permite a busca eficiente de forma análoga à busca binária em um vetor ordenado,
pois a cada passo a busca desce um nível na árvore,
reduzindo o espaço de busca proporcionalmente à sua altura \parencite[seç.~12.2]{clrs2022}.

A afirmação II é falsa.
A árvore encontra-se balanceada,
pois a diferença de altura entre a subárvore esquerda e a subárvore direita é de no máximo 1 \parencite[seç.~13.1]{clrs2022}.
Uma árvore binária só é considerada desbalanceada
quando a diferença entre as alturas das subárvores de algum nó é estritamente maior que 1 \parencite[cap.~13]{clrs2022}.

A afirmação III é falsa.
O caminhamento pós-ordem (\textit{postorder tree walk}) visita recursivamente a subárvore esquerda,
a subárvore direita e, por último, o nó raiz.
A sequência gerada (7, 14, 13, 10, 17, 25, 20, 15) não está disposta em ordem decrescente \parencite[seç.~12.1]{clrs2022}.

A afirmação IV é verdadeira.
O tempo de execução de uma busca em uma árvore binária de busca é limitado por sua altura $h$,
a qual em uma árvore balanceada é $h = O(\log n)$,
garantindo que o número de comparações seja $O(\log n)$ \parencite[seç.~12.2]{clrs2022}.
...
a função retorna \texttt{7}. Em ambos os casos as funções retornam o índice onde o valor 15 foi encontrado, assim, a impressão na linha 24 é de fato \texttt{7 - 7}.

A afirmação II é falsa. A busca sequencial da \texttt{funcao1} possui complexidade $O(n)$ no pior caso,
pois pode ser necessário percorrer todos os \texttt{N} elementos do vetor até encontrar (ou não) o valor procurado.
...
\begin{question}{2014-30}
Uma fazenda possui um único poço artesiano que deve abastecer $n$ bebedouros para o gado.
Deseja-se determinar um projeto de ligação entre esses $n+1$ pontos através de encanamentos com a menor extensão total.
Um algoritmo proposto para a solução do problema executa os seguintes passos:

\begin{small}
\begin{enumerate}
    \item Crie $n+1$ conjuntos unitários,
    cada um contendo um dos pontos a serem ligados entre si e insira esses conjuntos em um conjunto $C$.
    \item Crie um conjunto $D$ contendo um registro para cada combinação possível de dois pontos distintos a serem ligados.
    Cada registro deve conter os campos $c_i$,
    $c_j$ e $d$, em que $c_i$ e $c_j$ são os dois pontos a serem ligados e $d$ é a distância entre eles.
    \item Enquanto $D$ não estiver vazio faça:
    \begin{enumerate}
        \item[3.1.] Remova o registro de $D$ com o menor valor de distância $d$.
        \item[3.2.] Se os valores de $c_i$ e $c_j$ do registro removido pertencerem a conjuntos distintos de $C$, então:
        \begin{enumerate}
            \item[3.2.1.] Substitua estes dois conjuntos pela união entre eles.
            \item[3.2.2.] Guarde o registro removido em um conjunto-solução.
        \end{enumerate}
    \end{enumerate}
\end{enumerate}
\end{small}
...
\begin{small}
\begin{enumerate}[label=\Alph*)]
    \item o problema exemplifica a obtenção de uma árvore geradora mínima, portanto está no conjunto $P$.
    \item o algoritmo é uma heurística para o Problema do Caixeiro Viajante, logo apresenta complexidade polinomial.
    \item o problema descrito é de otimização, logo pertence ao conjunto $NP$-difícil, mas não ao conjunto $NP$-completo.
    \item uma alternativa para a solução do problema é usar o algoritmo de Dijkstra para obtenção do caminho mínimo entre dois pontos.
    \item o passo de maior custo do algoritmo é a criação do conjunto $D$ com as combinações de pontos,
    apresentando complexidade computacional $O(n!)$.
\end{enumerate}
\end{small}
\end{spacing}
\end{question}

O algoritmo apresentado descreve o algoritmo de Kruskal para a determinação
de uma árvore geradora mínima (MST) em um grafo ponderado e conexo \parencite[seç.~21.2]{clrs2022}.

A opção A está correta.
O problema consiste em conectar todos os vértices minimizando a extensão total dos encanamentos,
o que caracteriza a busca por uma MST \parencite[cap.~21]{clrs2022}.
Como o algoritmo de Kruskal resolve o problema em tempo polinomial $O(E \log V)$
(em que $E$ são o número de arestas e $V$ o número de vértices),
a questão pertence à classe $P$ \parencite[seç.~34.1]{clrs2022}.

A opção B está incorreta.
O algoritmo não é uma heurística para o Problema do Caixeiro Viajante (TSP),
mas sim o algoritmo exato de Kruskal para MST \parencite[seç.~21.2]{clrs2022}.

A opção C está incorreta.
Sendo um problema com solução exata de complexidade polinomial,
a MST pertence à classe $P$, e não ao conjunto $NP$-difícil \parencite[seç.~34.1]{clrs2022}.

A opção D está incorreta.
O algoritmo de Dijkstra resolve o problema de caminhos mínimos a partir de uma única origem \parencite[seç.~22.3]{clrs2022},
o que não garante a conexão de todos os vértices com custo total mínimo.

A opção E está incorreta.
Para $n+1$ vértices, o número máximo de pares não ordenados no conjunto $D$ é dado pela combinação
$\binom{n+1}{2} = \frac{(n+1)!}{2! \cdot ((n+1) - 2)!} = \frac{(n+1)n}{2} = O(n^2)$, e não por $O(n!)$ \parencite[seç.~C.1]{clrs2022}.
Logo, a opção correta é a A.

\begin{question}{2021-32}
Existe um grande número de implementações para algoritmos de ordenação.
Um dos fatores a serem considerados, por exemplo,
é o número máximo e médio de comparações que são necessárias para ordenar um vetor com $n$ elementos.
Diz-se também que um algoritmo de ordenação é estável se ele preserva a ordem de elementos que são iguais.
Isto é, se tais elementos aparecem na sequência ordenada na mesma ordem em que estão na sequência inicial.
Analise o algoritmo abaixo, onde \texttt{A} é um vetor e \texttt{i, j, lo e hi} são índices do vetor:
...
\begin{enumerate}[label=\Roman*.]
    \item O algoritmo precisa de um espaço adicional $O(n)$ para a pilha de recursão.
    \item O algoritmo apresentado é um algoritmo de ordenação recursivo e estável.
    \item O algoritmo precisa, em média, de $O(n \log n)$ comparações para ordenar n itens.
    \item O uso do primeiro elemento do vetor como ``pivot'' é mais eficiente que usar o último.
\end{enumerate}

\noindent
É correto apenas o que se afima em.
...
O algoritmo apresentado na questão é uma implementação do \textit{quicksort}, que utiliza a estratégia de divisão e conquista.
A função \texttt{particao} escolhe o último elemento do trecho do vetor, \texttt{A[hi]}, como pivô.
Em seguida, percorre os elementos do trecho, colocando antes do pivô aqueles que são menores que ele.
Ao final, troca o pivô com o elemento na posição \texttt{i} e retorna essa posição.
O algoritmo \texttt{ordena} então chama a si mesmo para ordenar recursivamente as duas partes do vetor.

A afirmação I é veradeira.
O algoritmo utiliza chamadas recursivas para ordenar as duas partições. No pior caso,
o pivô é sempre o menor ou o maior elemento do trecho considerado, produzindo uma partição vazia e outra com $n-1$ elementos.
Nesse cenário, a profundidade da recursão pode ser proporcional a $n$, exigindo até $O(n)$ de espaço adicional para armazenar
as chamadas na pilha de execução \parencite[seç.~7.2 e 7.4.1]{clrs2022}.

A afirmação II é falsa.
Embora o algoritmo seja recursivo, ele não é estável.
Um algoritmo de ordenação é estável quando preserva a ordem relativa de elementos com chaves iguais.
Neste algoritmo, elementos iguais ao pivô não são tratados como menores que ele,
mas as trocas realizadas durante a partição ainda podem alterar a ordem relativa desses elementos.

Por exemplo, considere dois elementos iguais, identificados como $2_a$ e $2_b$, no vetor
\[
    [2_a,\ 2_b,\ 1].
\]
O pivô é $1$. Ao final da partição, a troca do pivô com o primeiro elemento produz
\[
    [1,\ 2_b,\ 2_a],
\]
invertendo a ordem relativa dos dois elementos iguais. Portanto, o algoritmo não é estável.

A afirmação III é verdadeira.
A função \texttt{particao} percorre o trecho do vetor uma vez,
realizando trabalho linear no tamanho desse trecho.
Quando as partições são suficientemente equilibradas, a recorrência do quicksort é da forma
\[
    T(n) = 2T(n/2) + \Theta(n),
\]
cuja solução é $\Theta(n \log n)$.

\textcite{clrs2022} também demonstra que o tempo esperado do quicksort é $O(n \log n)$ sob uma escolha aleatória do pivô.
Para o algoritmo apresentado, que sempre escolhe o último elemento,
essa ordem média também é obtida quando se considera que as permutações da entrada são igualmente prováveis.
Portanto, a afirmação é verdadeira \parencite[seç.~7.4.2]{clrs2022}.

A afirmação IV é falsa.
Não é possível afirmar que escolher o primeiro elemento como pivô é sempre mais eficiente do que escolher o último.
O desempenho depende de como o pivô divide os elementos do vetor.
Uma escolha pode produzir partições equilibradas em uma entrada e partições muito desequilibradas em outra.

Além disso, escolher o primeiro elemento não elimina o pior caso:
em um vetor já ordenado em ordem crescente, por exemplo,
o primeiro elemento é o menor e gera uma partição vazia e outra com $n-1$ elementos.
O mesmo ocorre com a escolha do último elemento em uma entrada ordenada em ordem decrescente.
Assim, nenhuma das duas posições é universalmente mais eficiente \parencite[seç.~7.2]{clrs2022}.
Como apenas as afirmações I e III estão corretas, temos que A é a opção correta.
