...
Além disso, não é tão útil dizer que ``\textit{insertion sort} \emph{é} $O(n^2)$''; por mais que isso seja verdade,
poderíamos ter usado qualquer outro $O(n^c)$ em que $c \ge 2$ \parencite[57]{clrs2022}.
\pagebreak

\subsection{Exemplos}
Seguem agora questões resolvidas do Enade para exemplificar a análise de complexidade assintótica.
...
\begin{itemize}
    ...
    \item Na linha 10, a instrução original \texttt{a += pop(p);} remove o 5 do topo e soma ao valor atual da variável \texttt{a} (que é 5), resultando em \texttt{a = 10}. Se a instrução fosse substituída por \texttt{a += a;}, a variável iria realizar a operação \texttt{a = 5 + 5 = 10}.
\end{itemize}
\end{small}
...
a estrutura da pilha em si é alocada dinamicamente,
e o vetor \texttt{elementos} faz parte desse espaço de armazenamento alocado (conhecido também como \textit{heap}).
Conforme a especificação da linguagem C \parencite[seç. 7.24.3]{c23},
